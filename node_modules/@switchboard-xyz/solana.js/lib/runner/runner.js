...
import { SB_ATTESTATION_PID, SwitchboardProgram, } from "../SwitchboardProgram.js";
...
export class FunctionRunner {
    program;
    env;
    enclaveSigner = Keypair.generate();
    start = unixTimestamp();
    ...
    get functionData() {
        ...
        if (!this.env.functionData) {
            this.env.functionData = FunctionAccountData.fetch(this.program, this.env.functionKey)
                .then((data) => {
                if (!data) {
    ...
    get functionRequestData() {
        if (!this.env.functionRequestData) {
            this.env.functionRequestData = FunctionRequestAccountData.fetch(this.program, this.functionRequestKey)
                .then((data) => {
                if (!data) {
    ...
    static create(rpcEndpoint, commitment = "confirmed") {
        ...
        const connection = new Connection(rpcEndpoint ?? cluster === "localnet"
            ? "http://localhost:8899"
        ...
        const program = SwitchboardProgram.from(connection, undefined, undefined, switchboard);
        ...
        const functionData = env.FUNCTION_DATA && isHex(env.FUNCTION_DATA)
            ? Promise.resolve(FunctionAccountData.decode(Buffer.concat([
        ...
        const verifierEnclaveSigner = env.VERIFIER_ENCLAVE_SIGNER && isBase58(env.VERIFIER_ENCLAVE_SIGNER)
            ? new PublicKey(env.VERIFIER_ENCLAVE_SIGNER)
            : undefined;
        const queueAuthority = env.QUEUE_AUTHORITY && isBase58(env.QUEUE_AUTHORITY)
            ? new PublicKey(env.QUEUE_AUTHORITY)
            : undefined;
        // load function request info if provided
        const functionRequestKey = env.FUNCTION_REQUEST_KEY && isBase58(env.FUNCTION_REQUEST_KEY)
        ...
        const functionRequestData = !functionRequestKey
            ...
            : env.FUNCTION_REQUEST_DATA && isHex(env.FUNCTION_REQUEST_DATA)
                ? Promise.resolve(FunctionRequestAccountData.decode(Buffer.concat([
    ...
    async loadQueueAuthority() {
        if (!this.env.queueAuthority) {
            this.env.queueAuthority = await (async () => {
                const attestationQueueAccountData = await AttestationQueueAccountData.fetch(this.program, (await this.functionData).attestationQueue);
    ...
    async loadVerifierEnclaveSigner() {
        if (!this.env.verifierEnclaveSigner) {
            this.env.verifierEnclaveSigner = await (async () => {
                const verifierAccountData = await VerifierAccountData.fetch(this.program, this.verifier);
    ...
    async buildFnVerifyIxn(mrEnclave, errorCode = 0) {
        ...
        const verifierPermission = AttestationPermissionAccount.fromSeed(this.program, queueAuthority, functionData.attestationQueue, this.verifier);
        const verifierEnclaveSigner = await this.loadVerifierEnclaveSigner();
        return functionVerify(this.program, {
            params: {
    ...
    async buildFnRequestVerifyIxn(mrEnclave, errorCode = 0) {
        ...
        const verifierPermission = AttestationPermissionAccount.fromSeed(this.program, queueAuthority, functionData.attestationQueue, this.verifier);
        const verifierEnclaveSigner = await this.loadVerifierEnclaveSigner();
        return functionRequestVerify(this.program, {
            params: {
                observedTime: new BN(unixTimestamp()),
                errorCode: errorCode,
                mrEnclave: Array.from(mrEnclave),
