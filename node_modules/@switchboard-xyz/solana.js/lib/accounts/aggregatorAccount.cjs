...
class AggregatorAccount extends account_js_1.Account {
    ...
    get slidingWindowKey() {
        return web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("SlidingResultAccountData"), this.publicKey.toBytes()], this.program.oracleProgramId)[0];
    }
    ...
    static async createInstruction(program, payer, params, options) {
        ...
        ixns.push(web3_js_1.SystemProgram.createAccount({
            ...
            lamports: await program.connection.getMinimumBalanceForRentExemption(AggregatorAccount.size),
    ...
    async loadJobs(aggregator) {
        ...
        return await Promise.all(jobAccountDatas.map(async (j) => {
            ...
            if (!j.account.owner.equals(this.program.oracleProgramId)) {
                throw new errors.IncorrectOwner(this.program.oracleProgramId, j.account.owner);
            }
            const jobAccount = new jobAccount_js_1.JobAccount(this.program, j.publicKey);
            const jobState = JobAccountData_js_1.JobAccountData.decode(j.account.data);
            return {
                account: jobAccount,
                state: jobState,
