"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.RequestRoundNotActive = exports.RequestBufferFull = exports.ConfigParameterLocked = exports.FunctionRoutinesDisabled = exports.RoutineDisabled = exports.ContainerParamsTooLong = exports.RequestInvalidStatus = exports.InvalidParamsHash = exports.FunctionRequestNotReady = exports.AccountCloseNotReady = exports.AccountCloseNotPermitted = exports.RequestAlreadyInitialized = exports.FunctionCloseNotReady = exports.MissingFunctionAuthority = exports.UserRequestsDisabled = exports.FunctionNotReady = exports.InvalidRequest = exports.MissingFunctionEscrow = exports.InsufficientFunds = exports.RequestExpired = exports.IllegalExecuteAttempt = exports.InvalidAuthority = exports.InvalidEscrow = exports.IllegalVerifier = exports.InvalidQueue = exports.InvalidAddressLookupAddress = exports.InvalidResponder = exports.IncorrectMrEnclave = exports.InvalidSelfVerifyRequest = exports.InvalidVerifierIdx = exports.InvalidQuoteMode = exports.IncorrectObservedTime = exports.InsufficientLoadAmount = exports.InvalidReportData = exports.InvalidMrEnclave = exports.InvalidTimestamp = exports.InvalidConstraint = exports.PermissionDenied = exports.MrEnclaveAtCapacity = exports.MrEnclaveDoesntExist = exports.MrEnclaveAlreadyExists = exports.MrEnclavesEmpty = exports.InvalidSigner = exports.InvalidEnclaveSigner = exports.QueueFull = exports.InsufficientQueue = exports.InvalidNode = exports.QuoteExpired = exports.InvalidQuote = exports.GenericError = void 0;
exports.fromCode = exports.ServiceAlreadyAssignedToWorker = exports.ServiceWorkerEnclaveFull = exports.ServiceWorkerFull = exports.ServiceDisabled = exports.FunctionServicesDisabled = exports.MethodDeprecated = exports.RequestRoundAlreadyClosed = exports.MissingSbWalletAuthoritySigner = exports.EmptyEscrow = void 0;
class GenericError extends Error {
    logs;
    static code = 6000;
    code = 6000;
    name = "GenericError";
    constructor(logs) {
        super("6000: ");
        this.logs = logs;
    }
}
exports.GenericError = GenericError;
class InvalidQuote extends Error {
    logs;
    static code = 6001;
    code = 6001;
    name = "InvalidQuote";
    msg = "The provided enclave quote is invalid";
    constructor(logs) {
        super("6001: The provided enclave quote is invalid");
        this.logs = logs;
    }
}
exports.InvalidQuote = InvalidQuote;
class QuoteExpired extends Error {
    logs;
    static code = 6002;
    code = 6002;
    name = "QuoteExpired";
    msg = "The EnclaveAccount has expired and needs to be reverified";
    constructor(logs) {
        super("6002: The EnclaveAccount has expired and needs to be reverified");
        this.logs = logs;
    }
}
exports.QuoteExpired = QuoteExpired;
class InvalidNode extends Error {
    logs;
    static code = 6003;
    code = 6003;
    name = "InvalidNode";
    constructor(logs) {
        super("6003: ");
        this.logs = logs;
    }
}
exports.InvalidNode = InvalidNode;
class InsufficientQueue extends Error {
    logs;
    static code = 6004;
    code = 6004;
    name = "InsufficientQueue";
    msg = "The provided queue is empty and has no verifier oracles heartbeating on-chain.";
    constructor(logs) {
        super("6004: The provided queue is empty and has no verifier oracles heartbeating on-chain.");
        this.logs = logs;
    }
}
exports.InsufficientQueue = InsufficientQueue;
class QueueFull extends Error {
    logs;
    static code = 6005;
    code = 6005;
    name = "QueueFull";
    msg = "The provided queue is full and cannot support new verifiers";
    constructor(logs) {
        super("6005: The provided queue is full and cannot support new verifiers");
        this.logs = logs;
    }
}
exports.QueueFull = QueueFull;
class InvalidEnclaveSigner extends Error {
    logs;
    static code = 6006;
    code = 6006;
    name = "InvalidEnclaveSigner";
    msg = "The provided enclave_signer does not match the expected enclave_signer on the EnclaveAccount";
    constructor(logs) {
        super("6006: The provided enclave_signer does not match the expected enclave_signer on the EnclaveAccount");
        this.logs = logs;
    }
}
exports.InvalidEnclaveSigner = InvalidEnclaveSigner;
class InvalidSigner extends Error {
    logs;
    static code = 6007;
    code = 6007;
    name = "InvalidSigner";
    constructor(logs) {
        super("6007: ");
        this.logs = logs;
    }
}
exports.InvalidSigner = InvalidSigner;
class MrEnclavesEmpty extends Error {
    logs;
    static code = 6008;
    code = 6008;
    name = "MrEnclavesEmpty";
    msg = "This account has zero mr_enclaves defined";
    constructor(logs) {
        super("6008: This account has zero mr_enclaves defined");
        this.logs = logs;
    }
}
exports.MrEnclavesEmpty = MrEnclavesEmpty;
class MrEnclaveAlreadyExists extends Error {
    logs;
    static code = 6009;
    code = 6009;
    name = "MrEnclaveAlreadyExists";
    msg = "The MrEnclave value already exists in the array";
    constructor(logs) {
        super("6009: The MrEnclave value already exists in the array");
        this.logs = logs;
    }
}
exports.MrEnclaveAlreadyExists = MrEnclaveAlreadyExists;
class MrEnclaveDoesntExist extends Error {
    logs;
    static code = 6010;
    code = 6010;
    name = "MrEnclaveDoesntExist";
    msg = "The MrEnclave value was not found in the whitelist";
    constructor(logs) {
        super("6010: The MrEnclave value was not found in the whitelist");
        this.logs = logs;
    }
}
exports.MrEnclaveDoesntExist = MrEnclaveDoesntExist;
class MrEnclaveAtCapacity extends Error {
    logs;
    static code = 6011;
    code = 6011;
    name = "MrEnclaveAtCapacity";
    msg = "This account has a full mr_enclaves array. Remove some measurements to make room for new ones";
    constructor(logs) {
        super("6011: This account has a full mr_enclaves array. Remove some measurements to make room for new ones");
        this.logs = logs;
    }
}
exports.MrEnclaveAtCapacity = MrEnclaveAtCapacity;
class PermissionDenied extends Error {
    logs;
    static code = 6012;
    code = 6012;
    name = "PermissionDenied";
    msg = "The PermissionAccount is missing the required flags for this action. Check the queues config to see which permissions are required";
    constructor(logs) {
        super("6012: The PermissionAccount is missing the required flags for this action. Check the queues config to see which permissions are required");
        this.logs = logs;
    }
}
exports.PermissionDenied = PermissionDenied;
class InvalidConstraint extends Error {
    logs;
    static code = 6013;
    code = 6013;
    name = "InvalidConstraint";
    constructor(logs) {
        super("6013: ");
        this.logs = logs;
    }
}
exports.InvalidConstraint = InvalidConstraint;
class InvalidTimestamp extends Error {
    logs;
    static code = 6014;
    code = 6014;
    name = "InvalidTimestamp";
    constructor(logs) {
        super("6014: ");
        this.logs = logs;
    }
}
exports.InvalidTimestamp = InvalidTimestamp;
class InvalidMrEnclave extends Error {
    logs;
    static code = 6015;
    code = 6015;
    name = "InvalidMrEnclave";
    constructor(logs) {
        super("6015: ");
        this.logs = logs;
    }
}
exports.InvalidMrEnclave = InvalidMrEnclave;
class InvalidReportData extends Error {
    logs;
    static code = 6016;
    code = 6016;
    name = "InvalidReportData";
    constructor(logs) {
        super("6016: ");
        this.logs = logs;
    }
}
exports.InvalidReportData = InvalidReportData;
class InsufficientLoadAmount extends Error {
    logs;
    static code = 6017;
    code = 6017;
    name = "InsufficientLoadAmount";
    constructor(logs) {
        super("6017: ");
        this.logs = logs;
    }
}
exports.InsufficientLoadAmount = InsufficientLoadAmount;
class IncorrectObservedTime extends Error {
    logs;
    static code = 6018;
    code = 6018;
    name = "IncorrectObservedTime";
    msg = "The provided timestamp is not within the expected range. This may be indicative of an unhealthy enclave.";
    constructor(logs) {
        super("6018: The provided timestamp is not within the expected range. This may be indicative of an unhealthy enclave.");
        this.logs = logs;
    }
}
exports.IncorrectObservedTime = IncorrectObservedTime;
class InvalidQuoteMode extends Error {
    logs;
    static code = 6019;
    code = 6019;
    name = "InvalidQuoteMode";
    constructor(logs) {
        super("6019: ");
        this.logs = logs;
    }
}
exports.InvalidQuoteMode = InvalidQuoteMode;
class InvalidVerifierIdx extends Error {
    logs;
    static code = 6020;
    code = 6020;
    name = "InvalidVerifierIdx";
    constructor(logs) {
        super("6020: ");
        this.logs = logs;
    }
}
exports.InvalidVerifierIdx = InvalidVerifierIdx;
class InvalidSelfVerifyRequest extends Error {
    logs;
    static code = 6021;
    code = 6021;
    name = "InvalidSelfVerifyRequest";
    constructor(logs) {
        super("6021: ");
        this.logs = logs;
    }
}
exports.InvalidSelfVerifyRequest = InvalidSelfVerifyRequest;
class IncorrectMrEnclave extends Error {
    logs;
    static code = 6022;
    code = 6022;
    name = "IncorrectMrEnclave";
    msg = "The provided mr_enclave measurement did not match a value in its enclave settings. If you recently modified your function container, you may need to update the measurement in your FunctionAccount config.";
    constructor(logs) {
        super("6022: The provided mr_enclave measurement did not match a value in its enclave settings. If you recently modified your function container, you may need to update the measurement in your FunctionAccount config.");
        this.logs = logs;
    }
}
exports.IncorrectMrEnclave = IncorrectMrEnclave;
class InvalidResponder extends Error {
    logs;
    static code = 6023;
    code = 6023;
    name = "InvalidResponder";
    constructor(logs) {
        super("6023: ");
        this.logs = logs;
    }
}
exports.InvalidResponder = InvalidResponder;
class InvalidAddressLookupAddress extends Error {
    logs;
    static code = 6024;
    code = 6024;
    name = "InvalidAddressLookupAddress";
    msg = "The provided address_lookup_address did not match the expected address on-chain";
    constructor(logs) {
        super("6024: The provided address_lookup_address did not match the expected address on-chain");
        this.logs = logs;
    }
}
exports.InvalidAddressLookupAddress = InvalidAddressLookupAddress;
class InvalidQueue extends Error {
    logs;
    static code = 6025;
    code = 6025;
    name = "InvalidQueue";
    msg = "The provided attestation queue address did not match the expected address on-chain";
    constructor(logs) {
        super("6025: The provided attestation queue address did not match the expected address on-chain");
        this.logs = logs;
    }
}
exports.InvalidQueue = InvalidQueue;
class IllegalVerifier extends Error {
    logs;
    static code = 6026;
    code = 6026;
    name = "IllegalVerifier";
    constructor(logs) {
        super("6026: ");
        this.logs = logs;
    }
}
exports.IllegalVerifier = IllegalVerifier;
class InvalidEscrow extends Error {
    logs;
    static code = 6027;
    code = 6027;
    name = "InvalidEscrow";
    constructor(logs) {
        super("6027: ");
        this.logs = logs;
    }
}
exports.InvalidEscrow = InvalidEscrow;
class InvalidAuthority extends Error {
    logs;
    static code = 6028;
    code = 6028;
    name = "InvalidAuthority";
    msg = "The provided authority account does not match the expected value on-chain";
    constructor(logs) {
        super("6028: The provided authority account does not match the expected value on-chain");
        this.logs = logs;
    }
}
exports.InvalidAuthority = InvalidAuthority;
class IllegalExecuteAttempt extends Error {
    logs;
    static code = 6029;
    code = 6029;
    name = "IllegalExecuteAttempt";
    constructor(logs) {
        super("6029: ");
        this.logs = logs;
    }
}
exports.IllegalExecuteAttempt = IllegalExecuteAttempt;
class RequestExpired extends Error {
    logs;
    static code = 6030;
    code = 6030;
    name = "RequestExpired";
    msg = "The requests expiration_slot has expired";
    constructor(logs) {
        super("6030: The requests expiration_slot has expired");
        this.logs = logs;
    }
}
exports.RequestExpired = RequestExpired;
class InsufficientFunds extends Error {
    logs;
    static code = 6031;
    code = 6031;
    name = "InsufficientFunds";
    msg = "The escrow has insufficient funds for this action";
    constructor(logs) {
        super("6031: The escrow has insufficient funds for this action");
        this.logs = logs;
    }
}
exports.InsufficientFunds = InsufficientFunds;
class MissingFunctionEscrow extends Error {
    logs;
    static code = 6032;
    code = 6032;
    name = "MissingFunctionEscrow";
    msg = "The FunctionAccount escrow is required if function.requests_fee is greater than zero";
    constructor(logs) {
        super("6032: The FunctionAccount escrow is required if function.requests_fee is greater than zero");
        this.logs = logs;
    }
}
exports.MissingFunctionEscrow = MissingFunctionEscrow;
class InvalidRequest extends Error {
    logs;
    static code = 6033;
    code = 6033;
    name = "InvalidRequest";
    msg = "The provided requestSlot did not match the expected requestSlot on-chain. The request may have already been processed";
    constructor(logs) {
        super("6033: The provided requestSlot did not match the expected requestSlot on-chain. The request may have already been processed");
        this.logs = logs;
    }
}
exports.InvalidRequest = InvalidRequest;
class FunctionNotReady extends Error {
    logs;
    static code = 6034;
    code = 6034;
    name = "FunctionNotReady";
    msg = "The FunctionAccount status is not active (1)";
    constructor(logs) {
        super("6034: The FunctionAccount status is not active (1)");
        this.logs = logs;
    }
}
exports.FunctionNotReady = FunctionNotReady;
class UserRequestsDisabled extends Error {
    logs;
    static code = 6035;
    code = 6035;
    name = "UserRequestsDisabled";
    msg = "The FunctionAccount has set requests_disabled to true and disabled this action";
    constructor(logs) {
        super("6035: The FunctionAccount has set requests_disabled to true and disabled this action");
        this.logs = logs;
    }
}
exports.UserRequestsDisabled = UserRequestsDisabled;
class MissingFunctionAuthority extends Error {
    logs;
    static code = 6036;
    code = 6036;
    name = "MissingFunctionAuthority";
    msg = "The FunctionAccount authority is required to sign if function.requests_require_authorization is enabled";
    constructor(logs) {
        super("6036: The FunctionAccount authority is required to sign if function.requests_require_authorization is enabled");
        this.logs = logs;
    }
}
exports.MissingFunctionAuthority = MissingFunctionAuthority;
class FunctionCloseNotReady extends Error {
    logs;
    static code = 6037;
    code = 6037;
    name = "FunctionCloseNotReady";
    msg = "The FunctionAccount must have no requests before it can be closed";
    constructor(logs) {
        super("6037: The FunctionAccount must have no requests before it can be closed");
        this.logs = logs;
    }
}
exports.FunctionCloseNotReady = FunctionCloseNotReady;
class RequestAlreadyInitialized extends Error {
    logs;
    static code = 6038;
    code = 6038;
    name = "RequestAlreadyInitialized";
    msg = "Attempting to initialize an already created FunctionRequestAccount";
    constructor(logs) {
        super("6038: Attempting to initialize an already created FunctionRequestAccount");
        this.logs = logs;
    }
}
exports.RequestAlreadyInitialized = RequestAlreadyInitialized;
class AccountCloseNotPermitted extends Error {
    logs;
    static code = 6039;
    code = 6039;
    name = "AccountCloseNotPermitted";
    constructor(logs) {
        super("6039: ");
        this.logs = logs;
    }
}
exports.AccountCloseNotPermitted = AccountCloseNotPermitted;
class AccountCloseNotReady extends Error {
    logs;
    static code = 6040;
    code = 6040;
    name = "AccountCloseNotReady";
    constructor(logs) {
        super("6040: ");
        this.logs = logs;
    }
}
exports.AccountCloseNotReady = AccountCloseNotReady;
class FunctionRequestNotReady extends Error {
    logs;
    static code = 6041;
    code = 6041;
    name = "FunctionRequestNotReady";
    msg = "The FunctionRequestAccount is not ready to be verified";
    constructor(logs) {
        super("6041: The FunctionRequestAccount is not ready to be verified");
        this.logs = logs;
    }
}
exports.FunctionRequestNotReady = FunctionRequestNotReady;
class InvalidParamsHash extends Error {
    logs;
    static code = 6042;
    code = 6042;
    name = "InvalidParamsHash";
    msg = "The container params hash does not match the expected hash on-chain. The parameters may have been modified in-flight; the assigned oracle may need to pickup the account change before re-verifying the function.";
    constructor(logs) {
        super("6042: The container params hash does not match the expected hash on-chain. The parameters may have been modified in-flight; the assigned oracle may need to pickup the account change before re-verifying the function.");
        this.logs = logs;
    }
}
exports.InvalidParamsHash = InvalidParamsHash;
class RequestInvalidStatus extends Error {
    logs;
    static code = 6043;
    code = 6043;
    name = "RequestInvalidStatus";
    constructor(logs) {
        super("6043: ");
        this.logs = logs;
    }
}
exports.RequestInvalidStatus = RequestInvalidStatus;
class ContainerParamsTooLong extends Error {
    logs;
    static code = 6044;
    code = 6044;
    name = "ContainerParamsTooLong";
    msg = "Please ensure your parameters length is <= your account max length";
    constructor(logs) {
        super("6044: Please ensure your parameters length is <= your account max length");
        this.logs = logs;
    }
}
exports.ContainerParamsTooLong = ContainerParamsTooLong;
class RoutineDisabled extends Error {
    logs;
    static code = 6045;
    code = 6045;
    name = "RoutineDisabled";
    msg = "The routine has been disabled. Please check the routine's is_disabled status for more information.";
    constructor(logs) {
        super("6045: The routine has been disabled. Please check the routine's is_disabled status for more information.");
        this.logs = logs;
    }
}
exports.RoutineDisabled = RoutineDisabled;
class FunctionRoutinesDisabled extends Error {
    logs;
    static code = 6046;
    code = 6046;
    name = "FunctionRoutinesDisabled";
    msg = "The function authority has disabled routine execution for this function";
    constructor(logs) {
        super("6046: The function authority has disabled routine execution for this function");
        this.logs = logs;
    }
}
exports.FunctionRoutinesDisabled = FunctionRoutinesDisabled;
class ConfigParameterLocked extends Error {
    logs;
    static code = 6047;
    code = 6047;
    name = "ConfigParameterLocked";
    msg = "The configuration parameter has been locked and cannot be changed";
    constructor(logs) {
        super("6047: The configuration parameter has been locked and cannot be changed");
        this.logs = logs;
    }
}
exports.ConfigParameterLocked = ConfigParameterLocked;
class RequestBufferFull extends Error {
    logs;
    static code = 6048;
    code = 6048;
    name = "RequestBufferFull";
    constructor(logs) {
        super("6048: ");
        this.logs = logs;
    }
}
exports.RequestBufferFull = RequestBufferFull;
class RequestRoundNotActive extends Error {
    logs;
    static code = 6049;
    code = 6049;
    name = "RequestRoundNotActive";
    msg = "The request does not have an active round to verify";
    constructor(logs) {
        super("6049: The request does not have an active round to verify");
        this.logs = logs;
    }
}
exports.RequestRoundNotActive = RequestRoundNotActive;
class EmptyEscrow extends Error {
    logs;
    static code = 6050;
    code = 6050;
    name = "EmptyEscrow";
    msg = "The resources escrow token account has a balance of 0 and the queue reward is greater than 0";
    constructor(logs) {
        super("6050: The resources escrow token account has a balance of 0 and the queue reward is greater than 0");
        this.logs = logs;
    }
}
exports.EmptyEscrow = EmptyEscrow;
class MissingSbWalletAuthoritySigner extends Error {
    logs;
    static code = 6051;
    code = 6051;
    name = "MissingSbWalletAuthoritySigner";
    msg = "The SwitchboardWallet authority must sign this request in order to use its escrow wallet";
    constructor(logs) {
        super("6051: The SwitchboardWallet authority must sign this request in order to use its escrow wallet");
        this.logs = logs;
    }
}
exports.MissingSbWalletAuthoritySigner = MissingSbWalletAuthoritySigner;
class RequestRoundAlreadyClosed extends Error {
    logs;
    static code = 6052;
    code = 6052;
    name = "RequestRoundAlreadyClosed";
    msg = "The verifier is attempting to respond to an already closed request round with the same request_slot";
    constructor(logs) {
        super("6052: The verifier is attempting to respond to an already closed request round with the same request_slot");
        this.logs = logs;
    }
}
exports.RequestRoundAlreadyClosed = RequestRoundAlreadyClosed;
class MethodDeprecated extends Error {
    logs;
    static code = 6053;
    code = 6053;
    name = "MethodDeprecated";
    constructor(logs) {
        super("6053: ");
        this.logs = logs;
    }
}
exports.MethodDeprecated = MethodDeprecated;
class FunctionServicesDisabled extends Error {
    logs;
    static code = 6054;
    code = 6054;
    name = "FunctionServicesDisabled";
    msg = "The function authority has disabled service execution for this function";
    constructor(logs) {
        super("6054: The function authority has disabled service execution for this function");
        this.logs = logs;
    }
}
exports.FunctionServicesDisabled = FunctionServicesDisabled;
class ServiceDisabled extends Error {
    logs;
    static code = 6055;
    code = 6055;
    name = "ServiceDisabled";
    msg = "The service has been disabled. Please check the service's is_disabled status for more information.";
    constructor(logs) {
        super("6055: The service has been disabled. Please check the service's is_disabled status for more information.");
        this.logs = logs;
    }
}
exports.ServiceDisabled = ServiceDisabled;
class ServiceWorkerFull extends Error {
    logs;
    static code = 6056;
    code = 6056;
    name = "ServiceWorkerFull";
    msg = "The service worker already has the maximum number of services (128)";
    constructor(logs) {
        super("6056: The service worker already has the maximum number of services (128)");
        this.logs = logs;
    }
}
exports.ServiceWorkerFull = ServiceWorkerFull;
class ServiceWorkerEnclaveFull extends Error {
    logs;
    static code = 6057;
    code = 6057;
    name = "ServiceWorkerEnclaveFull";
    msg = "The service worker is already using its max enclave space for a set of services";
    constructor(logs) {
        super("6057: The service worker is already using its max enclave space for a set of services");
        this.logs = logs;
    }
}
exports.ServiceWorkerEnclaveFull = ServiceWorkerEnclaveFull;
class ServiceAlreadyAssignedToWorker extends Error {
    logs;
    static code = 6058;
    code = 6058;
    name = "ServiceAlreadyAssignedToWorker";
    msg = "Service is already being executed by a worker. Please remove the service before adding to a new service worker";
    constructor(logs) {
        super("6058: Service is already being executed by a worker. Please remove the service before adding to a new service worker");
        this.logs = logs;
    }
}
exports.ServiceAlreadyAssignedToWorker = ServiceAlreadyAssignedToWorker;
function fromCode(code, logs) {
    switch (code) {
        case 6000:
            return new GenericError(logs);
        case 6001:
            return new InvalidQuote(logs);
        case 6002:
            return new QuoteExpired(logs);
        case 6003:
            return new InvalidNode(logs);
        case 6004:
            return new InsufficientQueue(logs);
        case 6005:
            return new QueueFull(logs);
        case 6006:
            return new InvalidEnclaveSigner(logs);
        case 6007:
            return new InvalidSigner(logs);
        case 6008:
            return new MrEnclavesEmpty(logs);
        case 6009:
            return new MrEnclaveAlreadyExists(logs);
        case 6010:
            return new MrEnclaveDoesntExist(logs);
        case 6011:
            return new MrEnclaveAtCapacity(logs);
        case 6012:
            return new PermissionDenied(logs);
        case 6013:
            return new InvalidConstraint(logs);
        case 6014:
            return new InvalidTimestamp(logs);
        case 6015:
            return new InvalidMrEnclave(logs);
        case 6016:
            return new InvalidReportData(logs);
        case 6017:
            return new InsufficientLoadAmount(logs);
        case 6018:
            return new IncorrectObservedTime(logs);
        case 6019:
            return new InvalidQuoteMode(logs);
        case 6020:
            return new InvalidVerifierIdx(logs);
        case 6021:
            return new InvalidSelfVerifyRequest(logs);
        case 6022:
            return new IncorrectMrEnclave(logs);
        case 6023:
            return new InvalidResponder(logs);
        case 6024:
            return new InvalidAddressLookupAddress(logs);
        case 6025:
            return new InvalidQueue(logs);
        case 6026:
            return new IllegalVerifier(logs);
        case 6027:
            return new InvalidEscrow(logs);
        case 6028:
            return new InvalidAuthority(logs);
        case 6029:
            return new IllegalExecuteAttempt(logs);
        case 6030:
            return new RequestExpired(logs);
        case 6031:
            return new InsufficientFunds(logs);
        case 6032:
            return new MissingFunctionEscrow(logs);
        case 6033:
            return new InvalidRequest(logs);
        case 6034:
            return new FunctionNotReady(logs);
        case 6035:
            return new UserRequestsDisabled(logs);
        case 6036:
            return new MissingFunctionAuthority(logs);
        case 6037:
            return new FunctionCloseNotReady(logs);
        case 6038:
            return new RequestAlreadyInitialized(logs);
        case 6039:
            return new AccountCloseNotPermitted(logs);
        case 6040:
            return new AccountCloseNotReady(logs);
        case 6041:
            return new FunctionRequestNotReady(logs);
        case 6042:
            return new InvalidParamsHash(logs);
        case 6043:
            return new RequestInvalidStatus(logs);
        case 6044:
            return new ContainerParamsTooLong(logs);
        case 6045:
            return new RoutineDisabled(logs);
        case 6046:
            return new FunctionRoutinesDisabled(logs);
        case 6047:
            return new ConfigParameterLocked(logs);
        case 6048:
            return new RequestBufferFull(logs);
        case 6049:
            return new RequestRoundNotActive(logs);
        case 6050:
            return new EmptyEscrow(logs);
        case 6051:
            return new MissingSbWalletAuthoritySigner(logs);
        case 6052:
            return new RequestRoundAlreadyClosed(logs);
        case 6053:
            return new MethodDeprecated(logs);
        case 6054:
            return new FunctionServicesDisabled(logs);
        case 6055:
            return new ServiceDisabled(logs);
        case 6056:
            return new ServiceWorkerFull(logs);
        case 6057:
            return new ServiceWorkerEnclaveFull(logs);
        case 6058:
            return new ServiceAlreadyAssignedToWorker(logs);
    }
    return null;
}
exports.fromCode = fromCode;
